###############################################################################
# The original version is available from here:
#   https://github.com/zhpn1024/ribotish/blob/master/src/zbio/ribo.py
# This version is slightly modified to run the PSite pipeline.
###############################################################################
'''
...
from ribotish.zbio import stat, bam, gtf, exp, orf, tools, io, fa, interval
...
class Ribo: 
  ...
  def __init__(self, trans, ribobam = None, bamload = None, offset = offset, offdict = None, compatible = True, mis = 2, downsample = 1.0, seed = 1, saverid = False, paired = False):
    ...
    for r in bam.transReadsIter(ribobam, trans, compatible = compatible, mis = mis, maxNH = maxNH, minMapQ = minMapQ, secondary = secondary, paired = paired) : #ribobam.fetch_reads(trans.chr, trans.start, trans.stop):
      l = r.fragment_length()
      # mt1022: use PS tag attached by psite-predict (length filter should be performed by psite pipeline)
      off = r.get_tag('PS')  # offset(r, offdict)
...
def multiRiboGene(gene, bampaths, offdict = None, compatible = True, mis = 2, paired = False): 
  ...
  for i in range(len(bampaths)) :
    offunc = lambda r : r.genome_pos(r.get_tag('PS'))
    #def offunc(r):
