...
#   https://github.com/xryanglab/RiboCode/blob/master/RiboCode/process_bam.py
...
def read_bam(configData):
	...
	if os.path.exists(name + "_psites.hd5"):
		...
		for r in tracks.fetch(until_eof=True):
			...
			if r.query_length >= 25 and r.query_length <= 35:
				t_psite = r.reference_start + r.get_tag('PS')  # psites_dict[r.query_length]
				try:
